...
<c:if test="${fff}" >
	...
	<c:if test="${list.id == loginuser.u_id}" var="flg" />
	<c:if test="${flg}" >
	<tr>
	<td align=right>
	...
	<p>ID:<c:out value="${list.c_myid}"/> 名前:<c:out value="${list.c_myname}"/> 日付:<c:out value="${list.c_mytime}"/><br>
	<c:out value="${list.c_comment}"/></p>
	</div>
	</td>
	</tr>
